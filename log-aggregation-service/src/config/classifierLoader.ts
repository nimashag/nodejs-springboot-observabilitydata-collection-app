...
export function loadClassifierConfig(): ClassifierConfig {
  ...
  if (!fs.existsSync(configPath)) {
    console.warn(
      `Classifier config not found at ${configPath}, using default: rule-based`,
    );
  ...
  try {
    ...
    console.log(`✓ Loaded classifier config: ${config.classifier}`);
    return config;
  } catch (error) {
...
export function getConfiguredClassifierSync(): EventTypeClassifier {
  ...
  console.log(`✓ Using ${classifier.name} event type classifier`);
  return classifier;
}
    ...
    if (fs.existsSync(modelPath)) {
      try {
        ...
        if (
          ...
          classifier instanceof NaiveBayesEventTypeClassifier
        ) {
          const loaded = await NaiveBayesEventTypeClassifier.load(modelPath);
          return loaded;
        } else if (
          classifier.name === "nlp-based" &&
          classifier instanceof NLPEventTypeClassifier
        ) {
      ...
      console.warn(`⚠️  Model file not found at: ${modelPath}`);
      console.warn("Using untrained classifier");
    }
  }
