...
export class LogTemplateMiner {
  ...
  constructor() {
    ...
    this.eventTypeClassifier = getConfiguredClassifier();
  }
  ...
  async mineTemplates(
    ...
    const parameterizedLogs = logs.map(log => this.parameterizeLog(log));
   ...
   */
  private parameterizeLog(log: string): string {
