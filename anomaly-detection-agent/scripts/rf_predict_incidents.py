...
def worst_level(rows):
    """Pick the most severe level among rows (fatal > error > warn > info > debug)."""
...
def main(input_csv=DEFAULT_INPUT_CSV, model_path=DEFAULT_MODEL_PATH):
    ...
    # Build X (IMPORTANT: matches training column names)
    ...
    # -------- Group rows by request_id -> incidents --------
    incidents = []
    ...
    for request_id, rows in grouped.items():
        ...
        # highest score in that request group
        max_score = max(float(r.get("pred_anomaly_score", 0)) for r in rows)

        # most common service/event
        svc_counter = Counter()
        ev_counter = Counter()
