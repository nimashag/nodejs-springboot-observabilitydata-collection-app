...
function getApiBaseUrl(): string {
  ...
  console.log(
    "[LogAggregationAPI] Detected hostname:",
    hostname,
    "port:",
    currentPort,
    "Using API URL:",
...
export const getRootCause = async (
  ...
  const response = await api.get<RootCauseAnalysis>(
    `/api/traces/${traceId}/root-cause`,
  );
  return response.data;
};
...
export const getTemplates = async (
  ...
  const response = await api.get<{ count: number; templates: LogTemplate[] }>(
    "/api/templates",
    { params },
  );
...
export const mineTemplates = async (
  params: TemplateMiningParams,
...
export const matchTemplate = async (
  ...
  const response = await api.post<TemplateMatchResponse>(
    "/api/templates/match",
    request,
  );
  return response.data;
