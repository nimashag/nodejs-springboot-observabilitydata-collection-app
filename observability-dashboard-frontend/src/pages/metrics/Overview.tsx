...
function Sparkline({ values, height = 44 }: { values: number[]; height?: number }) {
  const w = 260;
  const h = height;
  if (!values.length) return <div className="text-xs text-gray-500 dark:text-gray-400">No data yet</div>;
  ...
  return (
    <svg className="text-blue-600 dark:text-blue-400" width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
...
function BarList({ items }: { items: Array<{ label: string; value: number }> }) {
  ...
  return (
    <div className="mt-2 grid gap-2">
      {items.map((it) => (
        <div key={it.label} className="grid grid-cols-[1fr_2fr_50px] items-center gap-3">
          <div className="truncate font-mono text-xs text-gray-800 dark:text-gray-200">{it.label}</div>
          ...
          <div className="text-right font-mono text-xs text-gray-500 dark:text-gray-400">{it.value}</div>
        </div>
      ))}
    </div>
  );
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          {health.error ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                ...
                <div className="text-xs text-gray-800 dark:text-gray-200">{health.data?.ok ? "ok" : "—"}</div>
              </div>

              <div className="flex items-center justify-between">
                <div className="text-xs text-gray-500 dark:text-gray-400">Last</div>
                <div className="font-mono text-xs text-gray-800 dark:text-gray-200">{formatTime(health.data?.ts)}</div>
              </div>

              {!settings.pollingEnabled ? (
                <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">Polling is OFF (Settings)</div>
              ) : null}
            </div>
          )}
        </Card>
          ...
          {signals.error ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                ...
                <div className="text-xs font-semibold text-gray-900 dark:text-gray-100">{criticalCount}</div>
              </div>

              <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                <div className="text-xs text-gray-500 dark:text-gray-400">Total signals</div>
                <div className="text-xs text-gray-800 dark:text-gray-200">{totalSignals}</div>
              </div>
        ...
        <Card title="KPI Coverage" subtitle="Coverage checker">
          {kpi.error ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                <div className="text-xs text-gray-500 dark:text-gray-400">Services missing KPIs</div>
                <div className="text-xs text-gray-800 dark:text-gray-200">{missingKpiServices}</div>
              </div>

              <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                <div className="text-xs text-gray-500 dark:text-gray-400">Services checked</div>
                <div className="text-xs text-gray-800 dark:text-gray-200">{kpi.data?.results?.length ?? 0}</div>
              </div>
          ...
          {plan.error ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                ...
                <div className="text-xs text-gray-800 dark:text-gray-200">{totalRules}</div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="text-xs text-gray-500 dark:text-gray-400">What it means</div>
                <div className="text-xs text-gray-800 dark:text-gray-200">Routes → KPIs</div>
              </div>
            </div>
          )}
        </Card>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card title="Trends" subtitle="Last samples (local history)">
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900/40">
              <div className="text-xs text-gray-500 dark:text-gray-400">Critical signals</div>
        ...
        <Card title="Breakdown" subtitle="What needs attention right now">
          <div className="grid gap-4">
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400">Signals by severity</div>
              <BarList items={sevBars} />
            </div>

            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400">Top services missing KPIs</div>
