...
function matchesDate(ts: number | undefined, selectedDate: string) {
  if (!selectedDate) return true;
  if (!ts) return false;
  return toDateInputValue(ts) === selectedDate;
}
  ...
  return (
    <button
      ...
      className={cx(
        ...
        "dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-200 dark:hover:bg-red-950/50",
        props.className,
      )}
    >
      {children}
...
export default function Signals({ settings }: { settings: AppSettings }) {
  ...
  const filtered = useMemo(() => {
    ...
    return raw
      ...
      .filter((s: any) => matchesDate(s.timestamp, dateFilter))
      .filter((s: any) => {
        if (!qq) return true;
        const blob =
          `${s.service} ${s.signal ?? ""} ${s.metric ?? ""} ${s.error ?? ""} ${s.route ?? ""}`.toLowerCase();
  ...
  }, [raw, q, svc, sev, dateFilter, sort]);

  const [selected, setSelected] = useState<any | null>(null);

  async function handleResetHistory() {
    const confirmed = window.confirm(
      "Delete all saved signal history? This cannot be undone."
    );
    if (!confirmed) return;

    try {
      setResetting(true);
      await api.resetSignalsHistory();
      setSelected(null);
      window.location.reload();
    } catch (err: any) {
      alert(`Failed to reset signal history: ${err?.message ?? "unknown error"}`);
    } finally {
      setResetting(false);
    }
  }
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
              ...
              onClick={() =>
                downloadCsv(
                  ...
                  (raw as any[]).map((s) => ({
                    ...
                    metric: s.metric ?? "",
                    route: s.route ?? "",
                    current: s.current ?? s.current_delta ?? "",
            ...
            <DangerButton onClick={handleResetHistory} disabled={resetting}>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            <TextInput
              value={q}
              onChange={setQ}
              placeholder="Filter... (service/signal/metric/route)"
            />
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">
            Generated: {formatDate(data?.generated_at)} {formatTime(data?.generated_at)}
          </div>
        </div>
        ...
        <div className="mt-5 overflow-hidden rounded-2xl bg-white dark:bg-gray-900">
          <div className="grid grid-cols-[180px_140px_140px_120px_1fr_130px_130px] gap-4 border-b border-gray-200 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:border-gray-800 dark:text-gray-400">
            <div>Service</div>
            <div>Type</div>
            <div>Severity</div>
            <div>Confidence</div>
          ...
          {filtered.slice(0, 300).length === 0 ? (
            <div className="px-5 py-4 text-sm text-gray-500 dark:text-gray-400">
              No matching signals ✅
            </div>
          ) : (
            filtered.slice(0, 300).map((s: any, idx: number) => (
              <div
                key={idx}
                onClick={() => setSelected(s)}
                className="grid cursor-pointer grid-cols-[180px_140px_140px_120px_1fr_130px_130px] gap-4 border-b border-gray-100 px-5 py-3 text-sm hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800/40"
              >
        ...
        <div className="mt-3 text-sm text-gray-500 dark:text-gray-400">
          Showing {Math.min(filtered.length, 300)} / {raw.length} signals
        </div>
      </div>
      ...
      >
        <div className="mt-4 space-y-3">
          ...
          {selected?.route ? (
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-950/40">
              <div className="text-xs text-gray-500 dark:text-gray-400">Route</div>
              <div className="mt-1 font-mono text-xs text-gray-800 dark:text-gray-100">
                {selected.route}
              </div>
            </div>
