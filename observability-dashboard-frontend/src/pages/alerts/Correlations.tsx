...
const Correlations = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const correlationData = await alertApiService.getCorrelations()
    ...
    return (
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-600 rounded-lg p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-red-800 dark:text-red-200 font-semibold">Error Loading Correlations</h3>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Alert Correlations & Incidents</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-1">Identify related alerts and complex incidents across services</p>
      </div>

      {/* Summary Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">

        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border-l-4 border-purple-500 dark:border-purple-400">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Incidents Detected</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{summary.total_incidents || incidents.length || 0}</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border-l-4 border-orange-500 dark:border-orange-400">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Services Affected</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{servicesAffected || 0}</p>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
            >
              <X className="w-4 h-4" />
              Clear
            </button>
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            {/* Correlation Strength Filter */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Correlation Strength</label>
              <select
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{incident.description || 'Multiple related alerts detected'}</p>
                ...
                {incident.root_cause_candidates && incident.root_cause_candidates.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                    <p className="text-xs font-medium text-gray-700 dark:text-gray-300">Possible Root Cause:</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        {filteredCorrelations.length === 0 ? (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            <LinkIcon className="w-12 h-12 mx-auto mb-3 text-gray-400 dark:text-gray-500" />
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredCorrelations.map((correlation: any, index: number) => {
                  ...
                  return (
                    <tr key={correlation.correlation_id || index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          <p className="font-medium text-gray-900 dark:text-white">{primaryAlert?.alert_type || primaryAlert?.alert_name || 'Alert A'}</p>
                          <p className="text-gray-500 dark:text-gray-400">↔ {correlation.related_alerts?.[0]?.alert_type || correlation.related_alerts?.[0]?.alert_name || 'Alert B'}</p>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm">
                          <p className="text-gray-900 dark:text-white">{primaryAlert?.service_name || 'N/A'}</p>
                          {relatedServices.size > 0 && Array.from(relatedServices).slice(1, 3).map((service, idx) => (
                            service && <p key={idx} className="text-gray-500 dark:text-gray-400 text-xs">+ {service}</p>
                          ))}
                          {relatedServices.size > 3 && (
                            <p className="text-gray-400 dark:text-gray-500 text-xs">+ {relatedServices.size - 3} more</p>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          strength === 'strong' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                          strength === 'moderate' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' :
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900 dark:text-white capitalize">{selectedIncident.severity || 'medium'}</p>
              </div>
              
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Description</p>
                <p className="text-gray-900 dark:text-white">{selectedIncident.description || 'Multiple related alerts detected'}</p>
              </div>
              
              {selectedIncident.root_cause_candidates && selectedIncident.root_cause_candidates.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Root Cause Candidates</p>
                  <div className="space-y-2 mt-2">
                    {selectedIncident.root_cause_candidates.map((rc: any, idx: number) => (
                      <div key={idx} className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                        <p className="text-gray-900 dark:text-white font-medium">{rc.service_name}: {rc.alert_type}</p>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{rc.reasoning}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Confidence: {(rc.confidence * 100).toFixed(1)}%</p>
              ...
              {selectedIncident.alerts && (
                <div>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Related Alerts ({selectedIncident.alerts.length})</p>
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {selectedIncident.alerts.map((alert: any, idx: number) => (
                      <div key={idx} className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg text-sm">
                        <p className="font-medium text-gray-900 dark:text-white">{alert.alert_name || alert.alert_type}</p>
                        <p className="text-gray-600 dark:text-gray-300">{alert.service_name}</p>
                      </div>
                    ))}
                  </div>
                </div>
