...
import { getTemplates, mineTemplates, deleteTemplate } from '../../api/logs/logAggregationApi';
import type { LogTemplate, TemplateMiningParams } from '../../types/logs/logAggregation.types';
import { format } from 'date-fns';
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronDown, ChevronUp, Clipboard, Copy } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Log Templates</h1>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-2 mb-2">
                        <p className="text-xs text-gray-500 dark:text-gray-400 font-mono">ID: {template.id}</p>
                        <button
                      ...
                      <p className="text-sm font-mono text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 p-3 rounded mb-2 break-all">
                        {template.template}
                      </p>
                      <div className="flex items-center space-x-4 text-xs text-gray-500 dark:text-gray-400 flex-wrap gap-2">
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      ...
                      <button
                        ...
                        className="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 text-sm font-medium px-3 py-1 rounded hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
                      >
                        Delete
                      </button>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Regex Pattern</p>
                          <button
                            onClick={() => copyToClipboard(template.pattern)}
                            className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 flex items-center space-x-1 text-xs"
                            title="Copy Pattern"
                          >
                        ...
                        <p className="text-xs font-mono text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 p-3 rounded break-all">
                          {template.pattern}
                        </p>
                      </div>
                      ...
                      {template.parameterizedLog && (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Parameterized Log</p>
                            <button
                      ...
                      {template.metadata && (
                        <div>
                          <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Metadata</p>
                          <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                            <div className="space-y-3">
                              {template.metadata.avgLength !== undefined && (
                                <div className="flex items-center">
                                  <span className="text-xs text-gray-600 dark:text-gray-400 font-medium w-32 flex-shrink-0">Average Length:</span>
                                  <span className="text-xs text-gray-900 dark:text-gray-100 font-semibold">{template.metadata.avgLength}</span>
                                </div>
                              )}
                              {template.metadata.parameterCount !== undefined && (
                                <div className="flex items-center">
                                  <span className="text-xs text-gray-600 dark:text-gray-400 font-medium w-32 flex-shrink-0">Parameter Count:</span>
                                  <span className="text-xs text-gray-900 dark:text-gray-100 font-semibold">{template.metadata.parameterCount}</span>
                                </div>
                              )}
                              {template.metadata.parameterTypes && Object.keys(template.metadata.parameterTypes).length > 0 && (
                                <div>
                                  <div className="text-xs text-gray-600 dark:text-gray-400 font-medium mb-2">Parameter Types:</div>
                                  <div className="space-y-2 pl-4 border-l-2 border-gray-300 dark:border-gray-600">
                                    {Object.entries(template.metadata.parameterTypes).map(([param, type]) => (
                                      <div key={param} className="flex items-center">
                                        <span className="text-xs text-gray-600 dark:text-gray-400 font-mono font-medium w-24 flex-shrink-0">{param}:</span>
                                        <span className="text-xs text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 px-2 py-1 rounded border border-gray-200 dark:border-gray-700">{type}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                          ...
                          <div className="space-y-2 max-h-96 overflow-y-auto">
                            {template.exampleLogs.map((example, index) => (
                              <div key={index} className="relative group">
                                <p className="text-xs font-mono text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 p-2 rounded break-all">
                                  {example}
                                </p>
                                <button
