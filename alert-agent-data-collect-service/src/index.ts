...
import { MLAgentResultsAggregator } from "./ml/ml-agent-results";
import { MLAgentResults, MLPrediction } from "./types";
...
function processAlertData() {
  ...
  console.log(
    `Collected: ${allAlerts.length} alerts from ${Object.keys(summary.alerts_by_service).length} services`,
  );
  ...
  collector.writeCombinedAlertHistory(
    allAlerts,
    path.join(outputDir, "combined-alert-history.json"),
  );
  ...
  const adjuster = new ThresholdAdjuster(
    allAlerts,
    analysisReport.service_baselines,
  );
  ...
  const { decisions: routingDecisions, summary: routingSummary } =
    router.routeAlerts(allAlerts);
  const routingEfficiency = router.calculateEfficiency(routingSummary);
  const routingRecommendations =
    router.generateRoutingRecommendations(routingSummary);
  ...
  } = suppressor.suppressAlerts(allAlerts);
  ...
  const correlationSummary = correlator.generateSummary(
    correlations,
    incidents,
  );
  ...
  const batchedNotifications = grouper.generateBatchedNotifications(
    deduplicationResult.groups,
  );

  const contextualAdjuster = new ContextualThresholdAdjuster(allAlerts);
  const contextualThresholds =
    contextualAdjuster.calculateContextualThresholds();
  const timeBasedPatterns = contextualAdjuster.analyzeTimeBasedPatterns();

  const remediationEngine = new RemediationEngine();
  const remediationSuggestions = remediationEngine.generateSuggestions(
    allAlerts.slice(0, 20),
  );
  const remediationMetrics = remediationEngine.getRemediationMetrics(
  ...
  return {
    summary: {
      ...
      false_positive_rate: (
        analysisReport.false_positive_analysis.estimated_fp_rate * 100
      ).toFixed(1),
      ...
      admin_notification_percentage: (
        (routingSummary.admin_notifications / routingSummary.total_alerts) *
        100
      ).toFixed(1),
...
async function processRealTimeAlert(alert: AlertEvent): Promise<void> {
  ...
  realTimeAlerts = realTimeAlerts.filter(
    (a) => a.normalized_timestamp > sevenDaysAgo,
  );
  ...
  if (mlClient) {
    try {
      ...
      if (mlPrediction && mlPrediction.success && mlPrediction.predictions) {
        ...
        console.log(
          `[ML] Priority: ${priority.priority_level} (${priority.priority_score.toFixed(1)}/100), TTR: ${ttr.ttr_minutes.toFixed(1)}min`,
        );
  ...
  console.log(
    `[REAL-TIME] Alert received: ${alert.alert_name} from ${alert.service_name} (${alert.alert_state})`,
  );
}
...
async function processIncrementalAnalysis(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[REAL-TIME] Running incremental analysis on ${allAlerts.length} alerts...`,
    );
    ...
    analysisData = {
      summary: {
        ...
        alerts_saved: impact.alerts_saved,
        noise_reduction:
          routingEfficiency.noise_reduction_percentage.toFixed(1),
        admin_notifications: routingSummary.admin_notifications,
        admin_notification_percentage: (
          (routingSummary.admin_notifications / routingSummary.total_alerts) *
          100
    ...
    collector!.writeSummary(
      summary,
      path.join(outputDir, "alert-summary.json"),
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        { suppressed, allowed, summary: suppressionSummary },
        null,
        2,
      ),
    );
    ...
    console.log(
      `[REAL-TIME] Incremental analysis complete. Total alerts: ${allAlerts.length}`,
    );
  } catch (error) {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && urlPath === "/api/apply-threshold") {
      ...
      req.on("end", async () => {
        try {
          const { service_name, alert_type, new_threshold, recommendation } =
            JSON.parse(body);

          if (!service_name || !alert_type || new_threshold === undefined) {
            res.writeHead(400);
            res.end(
              JSON.stringify({
                success: false,
                error:
                  "Missing required fields: service_name, alert_type, new_threshold",
              }),
            );
            return;
          }
          ...
          if (fs.existsSync(configPath)) {
            thresholdConfig = JSON.parse(fs.readFileSync(configPath, "utf-8"));
          }

          // Initialize service thresholds if not exists
          if (!thresholdConfig.thresholds[service_name]) {
            thresholdConfig.thresholds[service_name] = {};
          }

          // Store the old value for logging
          const oldValue = thresholdConfig.thresholds[service_name][alert_type];

          // Apply the new threshold
          thresholdConfig.thresholds[service_name][alert_type] = new_threshold;
          thresholdConfig.last_updated = new Date().toISOString();
          ...
          // Also track applied recommendations
          const appliedPath = path.join(outputDir, "applied-thresholds.json");
          let appliedHistory: any[] = [];
          if (fs.existsSync(appliedPath)) {
            try {
              const parsed = JSON.parse(fs.readFileSync(appliedPath, "utf-8"));
              // Ensure it's an array
              appliedHistory = Array.isArray(parsed) ? parsed : [];
            } catch {
              appliedHistory = [];
            }
          }
          ...
          fs.writeFileSync(
            appliedPath,
            JSON.stringify(appliedHistory, null, 2),
          );

          // Update in-memory analysisData if it exists
          if (analysisData && analysisData.threshold_config) {
            if (!analysisData.threshold_config.thresholds) {
              analysisData.threshold_config.thresholds = {};
            }
            if (!analysisData.threshold_config.thresholds[service_name]) {
              analysisData.threshold_config.thresholds[service_name] = {};
            }
            analysisData.threshold_config.thresholds[service_name][alert_type] =
              new_threshold;
          }

          console.log(
            `[THRESHOLD] Applied: ${service_name}.${alert_type}: ${oldValue} -> ${new_threshold}`,
          );
    ...
    if (req.method === "POST" && urlPath === "/api/send-email") {
      ...
      req.on("end", async () => {
        try {
          ...
          const mlReq = http.request(options, (mlRes: any) => {
            let mlBody = "";
            mlRes.on("data", (chunk: any) => {
              mlBody += chunk.toString();
            });
            mlRes.on("end", () => {
              res.writeHead(mlRes.statusCode || 200);
              res.end(mlBody);
            });
          });

          mlReq.on("error", (error: Error) => {
            console.error("[EMAIL] ML service error:", error);
            res.writeHead(500);
            res.end(
              JSON.stringify({
                success: false,
    ...
    if (req.method !== "GET") {
      ...
      res.end(
        JSON.stringify({
          error:
            "Method not allowed. Only GET, POST /api/alerts, POST /api/apply-threshold, POST /api/send-email, and OPTIONS are supported.",
        }),
      );
      return;
    }
    ...
    // GET /api/applied-thresholds - Get list of applied threshold recommendations
    if (urlPath === "/api/applied-thresholds") {
      const outputDir = path.join(__dirname, "..", "output");
      const appliedPath = path.join(outputDir, "applied-thresholds.json");
      ...
      res.writeHead(200);
      res.end(JSON.stringify(appliedHistory, null, 2));
      return;
    }
    ...
    if (!analysisData) {
      if (processingError) {
        ...
        res.writeHead(503);
        res.end(
          JSON.stringify({ error: "Analysis not yet complete. Please wait." }),
        );
      }
      return;
    }
    ...
    if (urlPath === "/" || urlPath === "/api/analysis") {
        ...
        JSON.stringify(
          {
            ...
            last_alert_time:
              realTimeAlerts.length > 0
                ? new Date(
                    realTimeAlerts[realTimeAlerts.length - 1]
      ...
      const mlReportPath = path.join(
        __dirname,
        "..",
        "ml-module",
        "models",
        "training_report_enhanced.json",
      );
      if (fs.existsSync(mlReportPath)) {
        ...
        res.end(
          JSON.stringify({
            error: "ML report not found. Please train models first.",
          }),
        );
      }
      ...
      res.writeHead(200);
      res.end(
        JSON.stringify(analysisData.correlations?.incidents || [], null, 2),
      );
        ...
        JSON.stringify(
          {
            adaptive_learning: {
              ...
              description:
                "Real-time threshold learning based on alert outcomes",
            ...
            correlation_analysis: {
              ...
              total_incidents:
                analysisData.correlations?.incidents?.length || 0,
              noise_reduction:
                analysisData.correlations?.summary?.noise_reduction,
            },
            predictive_alerting: {
              enabled: !!analysisData.predictions,
              description: "Forecast threshold breaches before they occur",
              total_predictions:
                analysisData.predictions?.predictions?.length || 0,
              avg_lead_time:
                analysisData.predictions?.metrics?.average_lead_time_minutes,
            },
            smart_deduplication: {
              enabled: !!analysisData.deduplication,
              description: "Intelligent alert grouping and batching",
              reduction_rate:
                analysisData.deduplication?.summary?.notification_reduction,
            ...
            contextual_thresholds: {
              ...
              total_contexts:
                analysisData.contextual_thresholds?.thresholds?.length || 0,
            },
            auto_remediation: {
              enabled: !!analysisData.remediation_suggestions,
              description: "Automated remediation suggestions",
              total_suggestions:
                analysisData.remediation_suggestions?.suggestions?.length || 0,
              auto_executable:
                analysisData.remediation_suggestions?.metrics?.auto_executable,
            },
            ab_testing: {
    ...
    } else if (urlPath === "/api/ml-agent-results") {
      // ML Agent Results - comprehensive ML processing results
      const mlResults = MLAgentResultsAggregator.generateResults(
        analysisData.alerts,
      );
      res.writeHead(200);
      res.end(JSON.stringify(mlResults, null, 2));
    } else if (urlPath === "/api/ml-classified-alerts") {
      // Get alerts with ML classifications
      const mlResults = MLAgentResultsAggregator.generateResults(
        analysisData.alerts,
      );
      res.writeHead(200);
      res.end(
        JSON.stringify(
          {
            summary: {
              total_classified: mlResults.summary.total_classified,
              by_priority: mlResults.classified_alerts.by_priority,
              by_type: mlResults.classified_alerts.by_type,
              by_service: mlResults.classified_alerts.by_service,
            },
            alerts: mlResults.recent_ml_alerts,
          },
          null,
          2,
        ),
      );
    } else if (urlPath === "/api/ml-predictions-results") {
      // Get ML prediction results (priority & TTR)
      const mlResults = MLAgentResultsAggregator.generateResults(
        analysisData.alerts,
      );
      res.writeHead(200);
      res.end(
        JSON.stringify(
          {
            summary: {
              total_predicted: mlResults.summary.total_predicted,
              high_priority_count: mlResults.summary.high_priority_count,
              avg_confidence: mlResults.summary.avg_confidence,
            },
    ...
    } else if (urlPath === "/api/ml-false-positives") {
      // Get detected false positives
      const mlResults = MLAgentResultsAggregator.generateResults(
        analysisData.alerts,
      );
      res.writeHead(200);
      res.end(
        JSON.stringify(
          {
            summary: {
              total_detected: mlResults.summary.false_positives_detected,
              suppressed_count: mlResults.summary.suppressed_count,
            },
            false_positives: mlResults.false_positives,
          },
          null,
          2,
        ),
      ...
      res.end(
        JSON.stringify({
          ...
          available_endpoints: [
            ...
            "/api/adaptive-config",
            "/api/ml-report",
            "/api/ml-agent-results",
            "/api/ml-classified-alerts",
            "/api/ml-predictions-results",
            "/api/ml-false-positives",
            "/api/realtime-stats",
...
(async () => {
  ...
  try {
    ...
    if (mlHealth && mlHealth.status === "healthy") {
      ...
      console.warn(
        "[!] ML prediction service not available - continuing without ML predictions",
      );
